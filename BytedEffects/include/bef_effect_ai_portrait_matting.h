...
/*
 * @brief 推荐配置  recommend configuration
 **/
...
/*
 * @brief SDK参数     param of SDK
 * edge_mode:
 *    算法参数，用来设置边界的模式    algorithm param, used to set boundary mode
 *    - 0: 不加边界     no boundary
 *    - 1: 加边界       has boundary
 *    - 2: 加边界, 其中, 2 和 1 策略不太一样，但效果上差别不大，可随意取一个    has boundary, has different strategy with 1
 * fresh_every:
 *    算法参数，设置调用多少次强制做预测，目前设置 15 即可      algorithm param, set forced prediction times, sample: 15
 * MP_OutputMinSideLen:
 *    返回短边的长度, 默认值为128, 需要为16的倍数；       length of min slide, default 128, must be multiple of 16
 * MP_OutputWidth 不设置，只做GetParam 兼容之前的调用    not set, only for compatible previous version
 * MP_OutputHeight 不设置，只做GetParam 兼容之前的接口；  not set, only for compatible previous version
 **/
...
/*
 * @brief 模型类型枚举    enum of model type
 **/
...
/*
 * @brief 返回结构体 return structure,
 * @param alpha 空间需要调用方负责分配内存和释放，保证有效的控场大于等于widht*height need malloc outer, ensure size >= width*height
...
struct bef_ai_matting_ret {
    unsigned char* alpha;  // alpha[i, j] 表示第 (i, j) 点的 mask 预测值，值位于[0, 255] 之间     alpha[i,j] represent predicted mask of point (i,j), between [0,255]
    int width;             // alpha 的宽度     with of alpha
    int height;            // alpha 的高度     height of alpha
};

/**
 * @brief 创建骨骼的句柄       create skeleton detect handle
...
/*
 * @brief 从文件初始化模型参数    initialize model param from file
 **/
...
bef_effect_ai_portrait_matting_get_param(
                                         ...
                                         int *value);
/**
 *  @brief 获取输出mask的尺寸      get size of mask
 *  @param handle 人体分割句柄    portrait matting handle
 *  @param width 输入图的宽度     with of image
 *  @param height 输入图的高度    height of image
 *  @param output_width[out] 输出mask的宽度  with of output mask
 *  @param output_height[out] 输出mask的高度 height of output mask
 *
 */
BEF_SDK_API bef_effect_result_t
bef_effect_ai_portrait_get_output_shape(bef_effect_handle_t handle,
                                        ...
                                        int *output_height);
/*
 * @brief 进行抠图操作    do detect
 * @note ret 结构图空间需要外部分配    must be allocated outer
 **/
BEF_SDK_API bef_effect_result_t
...
/**
 * @brief 人像分割授权    check and get license of portrait matting
