...
typedef struct bef_skeleton_point_info {
    float x; // 对应 cols, 范围在 [0, width] 之间      represent col, between [0, width]
    float y; // 对应 rows, 范围在 [0, height] 之间     represent row, between [0, height]
    bool is_detect; // 如果该值为 0, 则 x,y 无意义      x,y are invalid if is_detect is 0
} bef_skeleton_point_info;

typedef struct bef_skeleton_info {
    bef_skeleton_point_info      keyPointInfos[BEF_MAX_SKELETON_POINT_NUM];     // 检测到的骨骼信息     detected skeleton information
    bef_ai_rect  skeletonRect; // 骨骼rect    skeleton rect
} bef_skeleton_info;
...
/**
 * @brief 人体关键点检测   detect skeleton key points
...
/**
 * @brief 人体关键点授权   check and get license of skeleton detect
