...
/*
 *@brief 距离检测结果结构体      structure of distance detect result
 *@param
 *      face_count 有效的人脸个数      available face count
**/
typedef struct bef_ai_human_distance_result_st {
    float distances[BEF_MAX_FACE_NUM];
    int face_count;                         //有效的人脸个数，即表示attr_info中的前face_count个人脸是有效的      available face count, representing available count of attr_info
}bef_ai_human_distance_result;

/**
 * @brief 距离检测参数类型      type of distance detect param
 */
typedef  enum{
...
/**
 * 创建距离检测的句柄    create distance detect handle
 *@brief 初始化handle
...
/**
 * 设置距离检测参数     set distance detect param
 *@brief 初始化后的handle
 *@param [out] handle Created face verify handle
 *                     创建的人手检测句柄
 *
...
/**
 * @brief 距离检测授权    check and get license of distance detect
 * @param [in] handle Created face attribute detect handle
 *                    已创建的距离检测句柄
